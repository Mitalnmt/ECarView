...
function renderCarList() {
  ...
  let stt = 1;
  carList.forEach((car) => {
    if (car.done) return; // Bỏ qua xe đã done
    ...
    cell2.textContent = car.paid ? 'Đã thanh toán' : '';
    if (car.note) {
      cell2.textContent += ` (${car.note})`;
    }
    const cell3 = row.insertCell(2);
    let carCodeHtml = `${car.carCode}`;
    if (car.oldCarCodes && car.oldCarCodes.length > 0) {
      carCodeHtml += ` (${car.oldCarCodes.join(', ')})`;
    }
    cell3.textContent = carCodeHtml;
    const cell4 = row.insertCell(3);
    const timeOutFormatted = car.timeOut.toLocaleTimeString();
    const timeInFormatted = car.timeIn.toLocaleTimeString();
    cell4.textContent = `Ra: ${timeOutFormatted} / Vào: ${timeInFormatted}`;
    const cell5 = row.insertCell(4);
    cell5.textContent = getRemainingTime(car.timeIn, car);
    // Không còn cột Action
  });
}
...
function updateCountdowns() {
  ...
  renderCarList();
  setTimeout(updateCountdowns, 1000); // Đảm bảo cập nhật đếm ngược liên tục
}
...
loadSettings();
renderCarList();
updateCountdowns(); // Bắt đầu cập nhật đếm ngược
